...
processo_t *fila_remover_maior_prioridade(fila_t *fila);
...
static void so_escalona(so_t *self)
{
  ...
  if (self->processo_em_execucao != NULL && self->processo_em_execucao->estado == EXECUTANDO)
  {
    ...
    double t_exec = tempo_atual(self) - self->processo_em_execucao->metricas->tempo_criacao;
    
    double prioridade_nova = (processo_get_prioridade(self->processo_em_execucao) + t_exec/QUANTUM)/2;
    
    processo_set_prioridade(self->processo_em_execucao, prioridade_nova);
  ...
  processo_t *processo_maior_prioridade = fila_remover_maior_prioridade(self->fila_prontos);
  if(processo_maior_prioridade){
    if(self->processo_em_execucao != NULL){
      console_printf("SO: Escalonando processo... Sai processo %d, entra processo %d.", processo_get_pid(self->processo_em_execucao), processo_get_pid(processo_maior_prioridade));
        
    }else{
      console_printf("SO: Escalonando processo... entra processo %d.", processo_get_pid(processo_maior_prioridade));
      metricas_so_adiciona_tempo_ocioso(self->metricas, tempo_atual(self));
    }
    processo_set_estado(processo_maior_prioridade, EXECUTANDO);
    metricas_processo_incrementa_transicao(processo_maior_prioridade->metricas, EXECUTANDO);
    metricas_processo_para_tempo_estado(processo_maior_prioridade->metricas, self, EXECUTANDO);
    self->processo_em_execucao = processo_maior_prioridade;
    self->quantum_restante = QUANTUM;
    return;
  }
...
static void so_chamada_le(so_t *self)
{
  ...
  if (estado == 0)
  {
    ...
    double prioridade_nova = (processo_get_prioridade(self->processo_em_execucao) + t_exec/QUANTUM)/2;
    if(prioridade_nova < 0){
      console_printf("ERRO NEGATIVO\n");
      console_printf("velha %f texec %f\n", processo_get_prioridade(self->processo_em_execucao), t_exec);
    }
    processo_set_prioridade(self->processo_em_execucao, prioridade_nova);
...
processo_t *fila_remover_maior_prioridade(fila_t *fila) {
    ...
    nodo_fila_t *atual = fila->inicio;
    nodo_fila_t *maior = fila->inicio;

  
    while (atual) {
        if (processo_get_prioridade(atual->processo) > processo_get_prioridade(maior->processo)) {
            maior = atual;
    ...
    if (maior == fila->inicio) {
        fila->inicio = maior->proximo; 
    } else if (anterior_maior) {
    ...
    processo_t *processo = maior->processo;
    free(maior); 
    fila->tamanho--;
    return processo; 
}
